...
int[] bitSize = [sizeof(sbyte), sizeof(byte), sizeof(short), sizeof(ushort), sizeof(int), sizeof(uint), sizeof(long), 
...
for (int i = 0; i < numberTypes.Length; i ++)
    WriteLine("{0,-10} {1,-20} {2,30} {3,32}", numberTypes[i], bitSize[i], minValue[i], maxValue[i]);
WriteLine(dashedLine);